...
class UserApi extends Api{
	 ...
	 * 用户考试时间
	 */
	public function user_exam_time(){
		$type = $this->data[ 'type' ];
		if( $type == 1 ){
			$map[ 'uid' ] = $this->mid;
			$userexam = M( 'user_exam' )->where( $map )->order( 'time desc' )->find();
			if( $userexam ){
				return $userexam;
			} else {
				return array( 'status' => 1 , 'msg' => '没有结果' );;
			}
		} else {
			$data[ 'uid' ] = $this->mid;
			$data[ 'time' ] = t( $this->data[ 'time' ] );
			$res = M( 'user_exam' )->add( $data );
...
// 			$save ['password'] = md5 ( md5 ( $this->data ['password'] ) . $login_salt );
			...
			$updata = model( 'UserCenterOauth' )->update_password( $user[ 'login' ] , $this->data ['password'] , $this->data[ 'old_password' ] );
			if( $updata->status == 1 ){
				return array (
						'status' => 1,
						'msg' => '修改成功'
