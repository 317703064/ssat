...
abstract class Api {
    ...
    public function __construct($location=false) {
        $this->_module_white_list = array( 'Test','Search','Oauth','SsatArticle','SsatArticleTest','SsatPublicTest', 'Sitelist','Public' , 'Paper' , 'Article' ,'Words' ,'Grammar');
    ...
    private function verifyUser() {
    	...
    	$userdata = model( 'UserCenterOauth' )->auth_token( $verifycode['oauth_token'] , $verifycode['oauth_token_secret'] );
        if( $userdata->status == 1 ){
            $this->mid = $userdata->data->uid;
