...
//文章分类子集管理
admin.selectArticleCate = function(pid,obj,sid){    
    obj.nextAll().remove();
    if("undefined"==typeof(sid))
    	sid ='';
    if( pid > 0 ){
        //obj.after('<span>加载中……</span>');    //TODO 修改成图片
        $.post(U('admin/Article/selectArticleCate'),{pid:pid,sid:sid},function(msg){
            obj.nextAll().remove();
            if(msg.status=="1"){
                obj.after(msg.data);
            }
...
admin.addAnswer = function (id){
	if (typeof id === 'undefined') {
        return false;
    }
    ui.box.load(U('admin/PaperTest/doAnswer') + '&id=' + id, '疑惑解答');
    return false;
};

admin.delsatimg = function (id){
	if(confirm("确认删除")){
		location.href=U( 'admin/SatPublic/delImg' )+'&id='+id;
	}
};